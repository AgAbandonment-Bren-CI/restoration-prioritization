...
ssp2_pu <- read_sf("data/inputs/ssp2_pu.shp") %>% 
  clean_names() %>% 
...
# first, select the restoration method and land status you want to use
# selecting only costs for conducting natural regeneration (CAD)
costs_cnr <- costs %>% 
  filter(restoration_method == "conducting_natural_regeneration") %>% 
  filter(environmental_conditions == "CAD") %>% 
  select(!c(id))  #remove id column


# now merge the filtered cost with planning units
ssp2_pu_costs <- ssp2_pu %>% 
  full_join(costs_cnr, by = "biome_code") %>%             #joining objects by matching `biome_code` column values
