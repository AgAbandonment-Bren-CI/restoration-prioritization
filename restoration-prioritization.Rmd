...
## features for north
mask_feat_north <- function(carbon, bio, ssp, 
                           n, s, ne, se, cwest) {
  # mask carbon to SSP and region then convert
  carbon_pu_ssp <- terra::mask(carbon, ssp)
  carbon_north <- terra::mask(carbon_pu_ssp, n)
  carbon_north <- raster(carbon_north)
  # mask biodiversity to SSP then convert
  bio_pu_ssp <- terra::mask(bio, ssp)
  bio_north <- terra::mask(bio_pu_ssp, n)
  bio_north <- raster(bio_north)
  # stack the rastLayers
  feat_north <- stack(carbon_north, bio_north)
  return(feat_north)
}

ssp1_feat_n <- mask_feat_north(carbon_brazil, bio_ext_brazil, ssp1_abandoned_crop,
                              north, south, n_east, s_east, cent_west)
ssp2_feat_n <- mask_feat_north(carbon_brazil, bio_ext_brazil, ssp2_abandoned_crop,
                               north, south, n_east, s_east, cent_west)
ssp3_feat_n <- mask_feat_north(carbon_brazil, bio_ext_brazil, ssp3_abandoned_crop,
                               north, south, n_east, s_east, cent_west)
ssp4_feat_n <- mask_feat_north(carbon_brazil, bio_ext_brazil, ssp4_abandoned_crop,
                               north, south, n_east, s_east, cent_west)
ssp5_feat_n <- mask_feat_north(carbon_brazil, bio_ext_brazil, ssp5_abandoned_crop,
                               north, south, n_east, s_east, cent_west)


####### TEST ZONE ####################################################
## if same target for all zones, can put within matrix fxn
targ <- matrix(0.2, ncol = 2, nrow = 2)
## if different, can customize like this for 2 zones
targ[, 1] <- 0.2
targ[, 2] <- 0.2

z1 <- zones("zone 1" = ssp1_feat_n, "zone 2" = ssp1_feat_n)
...
test_stack <- stack(test_north, test_south)

p_test <- problem(test_stack, z1) %>% 
...
## features for south
mask_feat_south <- function(carbon, bio, ssp, 
                           n, s, ne, se, cwest) {
  # mask carbon to SSP and region then convert
  carbon_pu_ssp <- terra::mask(carbon, ssp)
  carbon_north <- carbon_pu_ssp %>% 
    crop(n) %>% 
    mask(n)
  carbon_north <- raster(carbon_north)
  # mask biodiversity to SSP then convert
  bio_pu_ssp <- terra::mask(bio, ssp)
  bio_north <- bio_pu_ssp %>% 
    crop(n) %>% 
    mask(n)
  bio_north <- raster(bio_north)
  # stack the rastLayers
