...
```{r}
rich <- rast("data/richness_current_allVerts.tif")
...
Test code for reclassifying to avoided extinction risk
```{r}
# read in values
...
# reclassify 
for (i in 1:nrow(extrisk)) {
  [  >= extrisk$rasterval[i, "lower"] & tr <= extrisk$rasterval[i, "upper"]] <- extrisk$extrisk[i]
}


# create matrix and df to reclassify biodiversity raster
reclass_df <- c(0, 0.08799, 0.00079,
                0.08899, 0.11099, 0.00080,
                0.11199, 0.11899, 0.00081,
                0.11999, 0.12599, 0.00082,
                0.12699, 0.13499, 0.00083,
                0.13599, 0.14299, 0.00084,
                0.14399, 0.14899, 0.00085,
                0.14999, 0.15599, 0.00086,
                0.15699, 0.16199, 0.00087)

reclass_m <- matrix(reclass_df, ncol = 3, byrow = TRUE)

# reclassify raster to extrisk
biodiversity_brazil_extrisk <- 
```
...
# mask feature data to only planning units
# for more accurate relative targets
biodiv_binary_pu <- 
...
## reset both targets to be minimize shortfall
## find how much carbon/biod is available in each biome and then set 
targets <- c(50000, 450000)
```

#### Target setting

To help determine our targets for the spatial prioritization, let's figure out how much of the biodiversity and carbon features are present in the planning units. We want to determine this both across all of Brazil and by biome. 

First, we'll calculate the total carbon sequestration potential across all planning units within Brazil, by SSP.
```{r}
## make list of rasters with planning units by SSP --------------------------
all_ssp_pu <- c("data/inputs/ssp1/SSP1_abandoned_cropland_brazil.tif",
                "data/inputs/ssp2/SSP2_abandoned_cropland_brazil.tif",
                "data/inputs/ssp3/SSP3_abandoned_cropland_brazil.tif",
                "data/inputs/ssp4/SSP4_abandoned_cropland_brazil.tif",
                "data/inputs/ssp5/SSP5_abandoned_cropland_brazil.tif")


## write function to return total carbon seq potential
## within the planning units in an SSP --------------------------------------
pu_carbon_brazil <- function(carbon, ssp) {
  #mask brazil carbon layer with SSP planning units
  carbon_brazil_ssp <- terra::mask(carbon, ssp) %>% 
  #calc the total carbon within planning units
  terra::global(., fun = 'sum', na.rm = TRUE)
  return(carbon_brazil_ssp)
}


## create empty df to store results -----------------------------------------
carbon_sum_brazil <- data.frame() 

## loop through each raster -------------------------------------------------
for(i in 1:length(all_ssp_pu)) {
  #load raster
  r <- rast(all_ssp_pu[i])
  #process raster and store result
  result <- pu_carbon_brazil(carbon_brazil, r)
  carbon_sum_brazil <- rbind(carbon_sum_brazil, data.frame(raster = all_ssp_pu[i],
                                           value = result,
                                           stringsAsFactors = FALSE))
}

## output results in table --------------------------------------------------
kable(carbon_sum_brazil) %>%
  kable_styling(full_width = F, font_size = 12) %>%
  column_spec(1, width = "20%") %>%
  kable_material_dark("striped") %>% 
  row_spec(0, font_size = 14, bold = T) 
```



We are these different???
```{r}
amazon_test <- mask(carbon_brazil, ssp1_abandoned_crop) %>% 
 mask(., cerrado, touches = TRUE) %>% 
  terra::global(., fun = 'sum', na.rm = TRUE)

brazil_test <- terra::global(blah, fun = 'sum', na.rm = T)

blah <- mask(carbon_brazil, ssp1_abandoned_crop)


test <- zonal(blah, biomes_rast, "sum", na.rm = TRUE)


glarb <- terra::extract(blah, biomes_vect,
                        fun = 'sum', method = 'simple', na.rm = T)
glarb


# try with biod

bio_test <- mask(biodiv_binary, ssp1_abandoned_crop)

glarb_bio <- terra::zonal(bio_test, biomes_rast, "sum", na.rm = T)

glarb_bio_sum <- sum(glarb_bio$BinaryBiodiversity, na.rm = T)
# move ahead with the zonal/extract one that 
plot(bio_test)
...
carbon_bio_pu_mask <- function(carbon, bio, ssp) {
  # mask brazil carbon layer with SSP pus
  carbon_pu_ssp <- terra::mask(carbon, ssp) 
  # convert to from spatrast to rastlayer
  carbon_pu_ssp <- raster(carbon_pu_ssp)
  
  # mask biodiversity layer by SSP pus
  bio_pu_ssp <- terra::mask(bio, ssp)
  # convert to rastLayer
...
ssp1_feat <- carbon_bio_pu_mask(carbon_brazil, biodiv_binary, ssp1_abandoned_crop)
ssp2_feat <- carbon_bio_pu_mask(carbon_brazil, biodiv_binary, ssp2_abandoned_crop)
ssp3_feat <- carbon_bio_pu_mask(carbon_brazil, biodiv_binary, ssp3_abandoned_crop)
ssp4_feat <- carbon_bio_pu_mask(carbon_brazil, biodiv_binary, ssp4_abandoned_crop)
ssp5_feat <- carbon_bio_pu_mask(carbon_brazil, biodiv_binary, ssp5_abandoned_crop)
```
...
```{r}
### test minimize shortfall
p_min_abs <- problem(ssp1_pu, ssp1_feat) %>% 
  add_min_shortfall_objective(1147.374261) %>% 
  add_absolute_targets(c(63804, 994741)) %>%  #this is 100% of targets in ssp1
  add_gurobi_solver(gap = 0.05)

s_min_abs <- solve(p_min_abs)

p_min_rel <- problem(ssp1_pu, ssp1_feat) %>% 
  add_min_shortfall_objective(1147.4) %>% 
  add_relative_targets(c(0.5, 0.5)) %>% 
  add_gurobi_solver(gap = 0.05)

s_min_rel <- solve(p_min_rel)
