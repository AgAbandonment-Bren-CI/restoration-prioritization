...
ssp1_amazon <- ssp1_abandoned_crop %>% 
  crop(amazon)

ssp1_caatinga <- ssp1_abandoned_crop %>% 
  crop(caatinga)

ssp1_cerrado <- ssp1_abandoned_crop %>% 
  crop(cerrado)

ssp1_atlantic_forest <- ssp1_abandoned_crop %>% 
  crop(atlantic_forest)

ssp1_pampa <- ssp1_abandoned_crop %>% 
  crop(pampa)

ssp1_pantanal <- ssp1_abandoned_crop %>% 
  crop(pantanal)
...
ssp1_abandoned_crop_cost <- raster(ssp1_abandoned_crop_cost)
```
...
```{r}
problem_ssp1 <- problem(ssp1_abandoned_crop_cost, 
                        bio_carbon) 
