...
### Biomes
Merge biomes with cost data then rasterize
```{r}
# first join with cost layer 
biomes_cost <- biomes %>% 
...
biomes_cost_rast <- rasterize(x = biomes_cost_vect,      #vect to rasterize
                              y = ssp1_abandoned_crop,   #raster to provide crs and spatial extent
...
```{r}
# test problem
p1 <- problem(ssp1_pu, bio_carbon) %>% 
  add_min_set_objective() %>% 
  add_relative_targets(0.1) %>% 
  add_gurobi_solver(gap = 0)


p2 <- problem(ssp2_pu, bio_carbon) %>% 
  add_min_set_objective() 


p3 <- problem(ssp3_pu, bio_carbon) %>% 
  add_min_set_objective() 


p4 <- problem(ssp4_pu, bio_carbon) %>% 
  add_min_set_objective() 


p5 <- problem(ssp5_pu, bio_carbon) %>% 
  add_min_set_objective() 
