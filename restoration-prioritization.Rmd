...
carbon_brazil <- mask(carbon_brazil, pantanal, inverse = TRUE, updatevalue = NA)
...
s_total <- (solve(p1) + solve(p2) + solve(p3) + solve(p4) + solve(p5))

ssp1_solution_total <- rast(s_total)
crs(ssp1_solution_total) = crs(ssp1_abandoned_crop)
...
total_solution[total_solution != 5] <- NA
total_cost <- mask(biomes_cost_rast, total_solution)
