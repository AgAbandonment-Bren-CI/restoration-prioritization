...
** NOTE: Try using that Lead functionfrom lab to move values up, then create matrix with 3 rows and reclassify.
```{r}
# read in values
...
biodiversity_brazil_extrisk <- classify(biodiversity_brazil, extrisk)
```
...
First, we will create a problem for each SSP that aims to minimize target shortfall while staying within a budget. Relative targets are set for carbon and biodiversity, higher than is possible within the budget. In the spatial solution, it will attempt to get as close to this unattainable target as possible while staying within the budget. For this run, the budget is 1147.37 million Brazilian Reals. This amount is the the entire budget for environmental preservation and conservation within the Brazilian Ministry of the Environment.
```{r}
# SSP1 --------------------------------------------
p1 <- problem(ssp1_pu, ssp1_feat) %>% 
  add_min_shortfall_objective(budget = 1147.37) %>% 
