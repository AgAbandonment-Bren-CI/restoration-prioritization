...
wdpa_shy <- rast("data/inputs/combined_WDPAs_Brazil_Dec.tif")
wdpa_5km <- rast("data/inputs/wdpa_5km_raster.tif")


#change biome crs because it's faster than changing crs of global raster
brazil_trans <- biomes_vect %>% 
  project(y = crs(wdpa_shy))

# crop PAs to terrestrial Brazil
wdpa_brazil <- wdpa_shy %>% 
  crop(brazil_trans) %>% 
  mask(brazil_trans) %>%  
...
wdpa_brazil <- resample(wdpa_brazil, ssp1_abandoned_crop, method = "bilinear")


wdpa_brazil[wdpa_brazil != 0] <- 1
wdpa_5km_brazil[wdpa_5km_brazil != 0] <- 1

test <- wdpa_5km_brazil - wdpa_brazil
...
Do something with current richness layer to compare to prioritizr results. 
...
-   NOTE: Testing adding PAs to ssp1 planning units and adjusting the features as necessary for the goals
```{r}
