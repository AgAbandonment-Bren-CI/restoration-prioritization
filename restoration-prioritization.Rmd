...
Test code for cropping to one biome and getting problem to run
```{r}
#### TEST CODE -------------------------
...
## Amazon still has 81,688,368 cells, so let's try looking at just pampa and see if that's under 1million pu
...
# problem
p_test_pampa <- problem(ssp1_pu_pampa, feat_pampa) %>% 
  add_min_set_objective() %>% 
  add_relative_targets(0.1) %>% 
  add_gurobi_solver(gap = 0.1)

# solve
s_test_pampa <- prioritizr::solve(p_test_pampa)
...
# check the forumulation of problem before trying to solve
presolve_check(p1)
